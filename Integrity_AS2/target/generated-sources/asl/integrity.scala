         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case integrity.this.adopt_achievement_innit_0 =>
                     integrity.this.adopt_achievement_innit_0.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
   ...
   object GoalParser extends IAgentGoalParser {
        override def create_goal_message(t: StructTerm, ref: IMessageSource) (implicit executionContext: ExecutionContext): Option[SubGoalMessage] = {
                                   if(t.matchOnlyFunctorAndArity("innit",0)) {
                                     val args: Parameters = Parameters(t.terms.toList)
                                     Option(SubGoalMessage(adopt_achievement_innit_0, args, ref))
                                   } else   {
      ...
      object adopt_achievement_innit_0 extends IGoal {
        ...
        def execute(params: Parameters) (implicit executionContext: ExecutionContext) : Unit = {
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
                 ...
                 //plan 0 start
                         ...
                         vars("Source").bind_to(StringTerm(executionContext.src.name))
                         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
                         val r0 = executionContext.beliefBase.query(StructTerm(",",Seq[GenericTerm](StructTerm(",",Seq[GenericTerm](StructTerm("perceptionOfIntentions",Seq[GenericTerm](vars("D1"),vars("I1"))),StructTerm("perceptionOfPrinciples",Seq[GenericTerm](vars("D2"),vars("I2"))))),StructTerm("weights",Seq[GenericTerm](vars("W1"),vars("W2"))))))
