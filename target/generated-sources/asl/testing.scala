package asl
 ...
 class testing  (coms: AgentCommunicationLayer = new  DefaultCommunications,
         ...
         message match {
            case SubGoalMessage(_,_,r) =>
               message.goal match {
                   ...
                   case testing.this.adopt_achievement_init_0 =>
                     testing.this.adopt_achievement_init_0.execute(message.params.asInstanceOf[Parameters])
 ...
 class Agent extends IAgent {
         ...
         override def agent_type: String = "testing"
         ...
         vars("Parent").bind_to(StringTerm(executionContext.parent.name))
             ...
             executionContext.src.asInstanceOf[AkkaMessageSource].address() ! IntentionErrorMessage(NoApplicablePlanMessage(),AkkaMessageSource(executionContext.agent.self))
                      ...
                      def plan0(vars: VarMap)(implicit executionContext: ExecutionContext): Unit = {
                                          ...
                                          PrimitiveAction.execute(PrimitiveAction.Parameters(() => println(StringTerm("hello"))))
...
object testing_companion { 
   def create() = new testing().agentBuilder 
   def create(in_coms : AgentCommunicationLayer) = new testing(coms = in_coms).agentBuilder 
   def create(in_beliefBaseFactory: IBeliefBaseFactory) = new testing(beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
   def create(in_coms : AgentCommunicationLayer, in_beliefBaseFactory: IBeliefBaseFactory) = new testing(coms = in_coms, beliefBaseFactory = in_beliefBaseFactory).agentBuilder 
} 
